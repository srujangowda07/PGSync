...
const Rent = () => {
  ...
  const togglePaid = (id) => {
    ...
    setData(updatedData);
    addLog(
      `Rent status toggled for ${resident.name}: ${!resident.paid ? "PAID" : "PENDING"}`,
    );
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 font-medium leading-relaxed">
            Automated rent calculation for the current cycle
          </p>
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {data.residents.length > 0 ? (
                    ...
                    >
                      ...
                      <td className="px-8 py-6 text-center">
                        <div className="flex justify-center gap-2">
                          {res.addons.wifi && (
                            <span
                              className="w-6 h-6 bg-blue-50 text-blue-600 rounded flex items-center justify-center text-[10px] border border-blue-100"
                              title="Wifi"
                            >
                              W
                            </span>
                          ...
                          {res.addons.mess && (
                            <span
                              className="w-6 h-6 bg-amber-50 text-amber-600 rounded flex items-center justify-center text-[10px] border border-amber-100"
                              title="Mess"
                            >
                              M
                            </span>
                          )}
                          {!res.addons.wifi && !res.addons.mess && (
                      ...
                      <td className="px-8 py-6 text-center">
                        <span
                          className={`px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.2em] inline-block border ${
                            res.paid
                ...
                <tr>
                  <td
                    colSpan="5"
                    className="px-8 py-20 text-center text-slate-300 italic font-medium"
                  >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-blue-600 p-8 rounded-[2rem] text-white flex flex-col justify-center">
          ...
          <p className="text-4xl font-black">
            ...
            {data.residents
              .filter((r) => !r.paid)
              .reduce((acc, res) => acc + calculateTotal(res), 0)
              .toLocaleString()}
          </p>
        </div>
      </div>
          ...
          <div className="bg-white rounded-[3rem] p-12 max-w-xl w-full relative z-10 shadow-2xl space-y-8 animate-in zoom-in-95">
            <div className="text-center">
              ...
              <p className="text-slate-500 font-bold mt-2">
                Connect your payment gateway for automated settlement.
              </p>
            </div>

            <div className="space-y-4">
              {["Stripe Connect", "Razorpay Standard", "PayPal Business"].map(
                (gatway) => (
                  <div
                    key={gatway}
                    className="flex justify-between items-center p-5 bg-slate-50 rounded-2xl border border-slate-100 group hover:border-blue-200 transition-all cursor-pointer"
                  >
